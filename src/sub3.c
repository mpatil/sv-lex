...
lchar           yycgidtbl[MAXNCG] = {
  ...
  '\n',				/* Newline is always special because '.'
				 * exclude it. */
...
int             ncg;		/* Should set to ncgidtbl*2; this is the
				 * largest value yycgid() */
		/*
		 * returns plus 1. 
		 */
...
remch(wchar_t c)
{
  ...
  if (!handleeuc) {
    if (!isascii(c))
      if (iswprint(c))
	warning
	    ("Non-ASCII character '%lc' in pattern; use -w or -e lex option.",
	     c);
      else
	warning
	    ("Non-ASCII character of value %#x in pattern; use -w or -e lex option.",
	     c);
    /*
  ...
  xlsearch(&lc, yycgidtbl, (unsigned *) &ncgidtbl, sizeof(lchar), cmplc);
}
