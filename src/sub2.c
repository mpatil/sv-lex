...
first(int v)
{
  ...
  case S2FINAL:
	...
	(name[parent[v]] != RSTR) && (check_me(v) == 0)) {
      break;
    }
    if (tmpstat[v] == FALSE) {
      count++;
...
packtrans(int st, CHR * tch, int *tst, int cnt, int tryit)
{
  ...
  for (i = 0; i < st; i++) {
	...
	break;
       ...
       */
      if (ast[j] != nexts[++p] ||
	  ast[j] == -1 || (cpackflg[st] && ach[j] != match[ach[j]]))
	diff++;
      j++;
    }
    while (ach[j]) {
...
layout(void)
{
  ...
  for (i = 0; i <= stnum; i++) {	/* for each state */
	...
	k = startup + ctable[nchar[j]];
	if (ctable[nchar[j]] <= 0)
	  printf("j %d nchar %ld ctable.nch %d\n",
  ...
  fprintf(fout, "//# define YYTYPE %s\n",
	  stnum + 1 >= NCH ? "int" : "byte unsigned");
  fprintf(fout, "YYWork yycrank[] = '{\n");
  for (i = 0; i <= yytop; i += 4) {
    for (j = 0; j < 4; j++) {
      k = i + j;
      if (verify[k])
  ...
  for (i = 0; i <= stnum; i++) {	/* for each state */
    ...
    if (sfall[i] != -1)
      fprintf(fout, "%d, ", sfall[i] + 1);	/* state + 1 */
    else
      fprintf(fout, "0, ");
    if (atable[i] != -1)
      fprintf(fout, "%d", atable[i]);
    else
      fprintf(fout, "0");
#ifdef DEBUG
    fprintf(fout, " },\n  /* state %d */", i);
  ...
  fprintf(fout, "int yytop_ptr = %d;\n", yytop);
  fprintf(fout, "//int yybgin_ptr = 1;\n");
  if (optim) {
    if (handleeuc) {
      fprintf(fout, "int yymatch[] = {\n");
    } else {
      fprintf(fout, "byte unsigned yymatch[] = {\n");
    }
   ...
   * put out yyextra 
   */
  fprintf(fout, "byte unsigned yyextra[] = {\n");
