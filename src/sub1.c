...
lgate(void)
{
  ...
  lgatflg = 1;
  if (fout == NULL) {
    sprintf(fname, "lex.yy.%c", ratfor ? 'r' : 'sv');
    fout = fopen(fname, "w");
  }
  if (fout == NULL)
...
sect1dump(void)
{
  ...
  if (chset == TRUE) {
	...
	putchar(' ');
	iswprint(ctable[i]) ?
	    putwc(ctable[i], stdout) : printf("%d", ctable[i]);
	putchar('\n');
      }
    }
  }
...
treedump(void)
{
  ...
  for (t = 0; t < tptr; t++) {
	...
	break;
      case DIV:
	printf("/ %ld %ld", (long) left[t], (long) right[t]);
	break;
      case BAR:
	printf("| %ld %ld", (long) left[t], (long) right[t]);
	break;
      case RCAT:
	printf("cat %ld %ld", (long) left[t], (long) right[t]);
	break;
      case PLUS:
	printf("+ %ld", (long) left[t]);
	break;
	...
	break;
      case RNEWE:
	printf("new %ld %ld", (long) left[t], (long) right[t]);
	break;
